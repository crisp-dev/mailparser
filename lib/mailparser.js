...
MailParser.prototype._process = function(finalPart) {
    ...
    if (!finalPart) {
        ...
        if (this._remainder.length > 1048576) {
            ...
            for (let i = 0; i < this._remainder.length; i += 1048576) {
                result += this._remainder.substr(i, 1048576) + '\n';
            }
            this._remainder = result.trimEnd(); // Remove trailing newline if present
        }
    }
...
MailParser.prototype._processStateBody = function(line) {
    ...
    // Too many multipart nodes in tree, do not even bother parsing the body
    if (this._multipartTree.length > 250) {
        return false;
    }

    // Handle multipart boundaries
    if (line.substr(0, 2) == "--") {
        // TODO: we need to limit how much we recurse here!
